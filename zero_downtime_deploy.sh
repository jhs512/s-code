...
current_service=$(ps aux | grep "socat TCP-LISTEN:$SOCAT_PORT" | grep -v grep | awk '{print $NF}')
...
if [[ -z "$current_service" ]]; then
  ...
  nohup socat "TCP-LISTEN:$SOCAT_PORT,fork" "TCP:localhost:$initial_port" &
...
# 포트 전환
kill -9 $(ps aux | grep "socat TCP-LISTEN:$SOCAT_PORT" | grep -v grep | awk '{print $2}')
nohup socat "TCP-LISTEN:$SOCAT_PORT,fork" "TCP:localhost:$next_port" &
