...
import static com.sbs.demo5.domain.bookModule.book.entity.QBook.book;

@RequiredArgsConstructor
public class BookRepositoryImpl implements com.sbs.demo5.domain.bookModule.book.repository.BookRepositoryCustom {
    ...
    private void applyKeywordFilter(String kwType, String kw, BooleanBuilder builder) {
        switch (kwType) {
            ...
            default -> builder.and(book.subject.containsIgnoreCase(kw).or(book.body.containsIgnoreCase(kw)));
        }
    }

    private JPAQuery<Book> createBooksQuery(BooleanBuilder builder) {
        return jpaQueryFactory.selectDistinct(book).from(book).where(builder);
    }
    ...
    private JPAQuery<Long> createTotalQuery(BooleanBuilder builder) {
        return jpaQueryFactory.select(book.count()).from(book).where(builder);
    }
}
