package com.ll.domain.baseModule.document.service;

import com.ll.domain.baseModule.document.standard.Document;
...
public class DocumentService {
    ...
    // 임시파일을 특정 admMember 와 연관지어서 저장한 이유는 혹시나 있을지 모르는 대량 업로드 공격발생시 추적을 위해서 이다.
    ...
    public void updateTempGenFilesToInBody(Document doc) {
        ...
        String newBody = Ut.str.replace(doc.getBody(), "\\(" + TEMP_FILE_PATH + "/([^)]+)\\?type=temp\\)", (String url) -> {
            ...
            String newUrl = genFileService.tempToFile(url, (BaseEntity) doc, "global", "inBody", 0).getUrl();
