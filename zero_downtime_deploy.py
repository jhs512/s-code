...
class ServiceManager:
    # 초기화 함수
    def __init__(self, socat_port: int = 8081, sleep_duration: int = 20) -> None:
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 's_code_1_2', self.services['s_code_1_2']
        else:
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run --name={name} -p {port}:8080 -v /docker_projects/s_code_1/volumes/gen:/gen --restart unless-stopped -e TZ=Asia/Seoul --pull always -d ghcr.io/jhs512/s-code-1")
